...
function normalize(s){
  ...
  s=merge(s,DEFAULT);s.version=55;s.settings=s.settings&&typeof s.settings==='object'?s.settings:clone(DEFAULT.settings);s.settings.fontScale=clamp(Number(s.settings.fontScale)||1.06,.98,1.12);
...
function totalProfitAll(){return (state.profits||[]).reduce((a,x)=>a+Math.max(0,Number(x.amount)||0),0)}
function totalExpenseAll(){return (state.expenses||[]).reduce((a,x)=>a+Math.max(0,Number(x.amount)||0),0)}
function walletMetrics(){
  const opening=Math.max(0,Number(state.wallet?.openingBalance)||0);
  const historicalGoal=Math.max(0,Number(state.goal?.finance?.manualSavedAmount)||0);
  const income=totalProfitAll(),expense=totalExpenseAll();
  const capital=opening+historicalGoal+income-expense;
  const goal=totalGoalSaved();
  const cash=capital-goal;
  return {opening,historicalGoal,income,expense,capital,goal,cash};
}
function totalGoalSaved(){return goalFinanceMetrics().saved}
function currentMoneyPosition(){return walletMetrics().cash}
function totalCapital(){return walletMetrics().capital}
