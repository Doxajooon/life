...
assert.equal(pkg.version, "56.0.0", "package version must match V56 CLEAN");
...
assert.equal(scriptMatches.length, 3, "index.html must load exactly three application scripts");
const supabaseJs = read("public/js/supabase.js");
...
]) assert.ok(runtimeJs.includes(required) || html.includes(required), `missing ${required}`);
...
assert.ok(runtimeJs.includes("cloudNetworkState"), "cloud network status missing");
...
assert.ok(html.includes("goalAiCard") && runtimeJs.includes("requestGoalAI"), "goal AI panel missing");
...
assert.ok(runtimeJs.includes("Пророк Мухаммад ﷺ"), "Prophet motivation quotes missing");
...
assert.ok(html.includes("pulseDayCard") && runtimeJs.includes("dayProgressMetrics") && css.includes(".runner-runner"), "interactive day progress runner missing");
assert.ok(html.includes("authProgressText") && html.includes("authProgressBar") && css.includes(".auth-spinner"), "auth progress UI missing");
...
assert.ok(runtimeJs.includes("const stored=cloudAuth()"), "persistent Supabase session restore missing");
assert.ok(runtimeJs.includes("async function cloudLogout()"), "explicit cloud logout function missing");
...
assert.ok(runtimeJs.includes("function effectiveDayMode()"), "automatic day mode logic missing");
assert.ok(runtimeJs.includes("function dayMood()"), "automatic day mood logic missing");
assert.ok(runtimeJs.includes("function openMetricDetail("), "interactive KPI detail modal missing");
assert.ok(runtimeJs.includes("function playNotificationSound()"), "in-app notification sound missing");
assert.ok(runtimeJs.includes("cloudLogout()"), "manual logout action missing");
...
assert.ok(runtimeJs.includes("stateFingerprint"), "change-driven persistence fingerprint missing");
assert.ok(runtimeJs.includes("async function importJSON"), "safe JSON import missing");
assert.ok(runtimeJs.includes("async function forgetDeletedNoteFromSnapshots"), "permanent note snapshot cleanup missing");
...
assert.ok(runtimeJs.includes("_cloudBaseUpdatedAt"), "cloud base revision guard missing");
assert.ok(runtimeJs.includes("Данные обновлены в облаке"), "cloud conflict protection notice missing");
